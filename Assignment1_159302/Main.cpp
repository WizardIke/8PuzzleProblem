...
void AnimateSolution(string const& initialState, string const& goalState, string const& path);
...
void AnimateSolution(string const& initialState, string const& goalState, string const& path){
	...
	Puzzle p1(initialState, goalState);
	Puzzle p2;
	Puzzle *p = &p1;
	Puzzle *nextState = &p2;
	
    displayBoard(p->getString());
	...
	for(unsigned int i = 0; i < path.length(); i++){
      ...
      displayBoard(nextState->getString());
...
//-----------------------------------------------------------------------------
	...
	else if(path == "OOM") cout << "\n\n*---- NO SOLUTION found. (Out of memory!) ----*" << endl;
